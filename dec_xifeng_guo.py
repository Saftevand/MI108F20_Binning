...
import metrics_xifeng_guo as metrics
...
def autoencoder(dims, act='relu', init='glorot_uniform'):
    ...
    # hidden layer
    h = keras.layers.Dense(dims[-1], kernel_initializer=init, name='encoder_%d' % (n_stacks - 1))(h)  # hidden layer, features are extracted from here
    ...
    return keras.models.Model(inputs=x, outputs=y, name='AE'), keras.models.Model(inputs=x, outputs=h, name='encoder')
...
class ClusteringLayer(Layer):
    ...
    def build(self, input_shape):
        ...
        self.input_spec = InputSpec(dtype=keras.backend.floatx(), shape=(None, input_dim))
...
class DEC(object):
    ...
    def fit(self, x, y=None, maxiter=2e4, batch_size=258, tol=1e-3,
...
if __name__ == "__main__":
    ...
    '''
    # Vores egen dataprocessor til at hente vores eget data
    feature_matrix = data_processor.get_featurematrix(use_depth=True, load_data=True)
    x, _ = data_processor.get_train_and_validation_data(feature_matrix=feature_matrix, split_value=1)

    # Vi har ingen labels så y = _ = None
    y = None

    '''Vi ved ikke hvor mange clusters vi vil have'''
    ...
    '''
    update_interval of pretrain_epochs er ikke assigned.
    Test værdier below
    '''
    update_interval = 140
    pretrain_epochs = 200
    ...
    dec = DEC(dims=[input_shape, 500, 500, 2000, 10], n_clusters=n_clusters, init=init)
    ...
    dec.model.summary()
    t0 = time()
    dec.compile(optimizer=keras.optimizers.SGD(0.01, 0.9), loss='kld')
